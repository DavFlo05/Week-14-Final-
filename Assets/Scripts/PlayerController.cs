...
public class PlayerController : MonoBehaviour
{
    ...
    public void LoseALife()
    {
        if (shieldActive)
        {
            return; // Don't lose a life if the shield is active
        }
        lives--;
        gameManager.ChangeLivesText(lives);
        if (lives == 0)
    ...
    {
        if (shieldActive) yield break; // Prevent overlapping shields

        shieldActive = true;
        shieldPrefab.SetActive(true); // Show shield visual

        yield return new WaitForSeconds(3f);

        shieldActive = false;
        shieldPrefab.SetActive(false); // Hide shield visual
